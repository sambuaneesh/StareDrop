...
impl Reassembler for BasicReassembler {
    fn accept_chunk(&mut self, chunk: Chunk) -> Result<ChunkAcceptResult> {
        ...
        match self.total_chunks {
            ...
            Some(existing) if existing != chunk.total_chunks => {
                return Err(StareDropError::InvalidData(format!(
    ...
    fn reconstruct(&self) -> Result<Vec<u8>> {
        if !self.is_complete() {
            return Err(StareDropError::Failed("transfer not complete".to_string()));
        }
        ...
        for idx in 0..total {
            let payload = self
                .chunks
                .get(&idx)
                .ok_or_else(|| StareDropError::NotFound(format!("missing chunk {}", idx)))?;
